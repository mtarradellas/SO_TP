...
#define LEVELS (20 - 7 + 1)  // 1???? //each power from 7 to 20 will be a level (each with a different size of block). starting from seven because the smallest block will be of size 2^7 and the max will be the whole memory 2^20
...
/*
 * Reserves a block of contiguous memory of size equal to (or the size of first power of 2 bigger than) the argument (there are no controls of incorrect use of memory,
 * it is the responsibility of the user to respect the limits requested).
 ...
 * Reserves a block of contiguous memory of size equal to (or the size of first power of 2 bigger than) the argument (there are no controls of incorrect use of memory,
 * it is the responsibility of the user to respect the limits requested).
 * It also receives a pointer to a block of memory to be copied to the new block. The old block is released.
