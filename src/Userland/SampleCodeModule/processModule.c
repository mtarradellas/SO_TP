...
unsigned long int createProcess(char* name, int (*entry)(int, char**), int argc,
  ...
  unsigned long int pid =
      systemCall((uint64_t)CREATEPROC, (uint64_t)name, (uint64_t)entry,
...
unsigned long int setProcess(char* name, int (*entry)(int, char**), int argc,
                             char** argv, int priority) {
  unsigned long int pid =
      systemCall((uint64_t)SETPROCESS, (uint64_t)name, (uint64_t)entry,
                 (uint64_t)argc, (uint64_t)argv, (uint64_t)priority);
  return pid;
}

void pipe(int fd[2]) { systemCall((uint64_t)PIPE, (uint64_t)fd, 0, 0, 0, 0); }
