...
void drawCircle(Color color, int radio, int x, int y){
	systemCall((uint64_t)DRAWCIRCLE, (uint64_t) &color, (uint64_t)&radio, (uint64_t)&x, (uint64_t)&y, 0);
}

void drawRectangle(Color color, int x, int y, int b, int h){
	systemCall((uint64_t)DRAWRECTANGLE, (uint64_t)&color, (uint64_t) &b, (uint64_t)&h, (uint64_t) &x, (uint64_t)&y);
}
